...
- PROFILE THEN PRODUCTS (category-aware): For a fitted mobility product, work out WHICH category it is, then focus on the questions that matter most for THAT category. Do NOT ask a fixed set for everything — different products are fitted on different details. The most relevant questions per category:
    - Walking aids (sticks, crutches, canes): any condition or limitation (e.g. balance, weak grip, arthritis); the user's height (sets the correct length) and approximate weight; whether they need support on one side or both; indoor, outdoor, or both.
    - Rollators / walkers: any condition or limitation (e.g. balance, weak grip); the user's approximate weight (sets the safe maximum user weight) and height (handle height must match); indoor, outdoor, or both; whether they need a seat to rest on and whether it must fold for a car.
    ...
    - Mobility scooters: any condition or limitation (e.g. how far they can walk, balance); the user's weight; where it will be used (pavement, road, or both) and the distance/range needed; how it will be stored or transported (car boot, garage).
